...
type status struct {
	...
	version                             string
	githash                             string
	shortMemoryWriter, longMemoryWriter memorywriter.LogWriter
}
...
func ServeStatus(r *mux.Router, c core.CoreInf, v, h string, mw, dmw memorywriter.LogWriter) {
	status := &status{
		core:              c,
		version:           v,
		githash:           h,
...
func makeStatusTemplateDevice(dev trezordcore.EnumerateEntry) statusTemplateDevice {
	var session string
	if dev.Session != nil {
		session = *dev.Session
	}
